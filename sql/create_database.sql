...
create table public.saved_jobs (
  saved_job_id bigint generated by default as identity primary key,
  job_id bigint references public.jobs not null,
  freelancer_id bigint references public.freelancers not null,
  time_sent bigint not null
);
...
create policy "Freelancers can save a job" on public.saved_jobs
  for insert with check (
    freelancer_id = (
      select freelancer_id from public.freelancers
      where user_id  = auth.uid()
...
create policy "Freelancers can look at the jobs that they have saved" on public.saved_jobs
  for select using (
    freelancer_id = (
      select freelancer_id from public.freelancers 
      where user_id = auth.uid()
